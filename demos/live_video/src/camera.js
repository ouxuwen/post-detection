...
export class Camera {
  ...
  drawResult(pose) {
    if (pose.keypoints != null) {
      ...
      // window.keypoints = pose.keypoints;
      // console.log(pose.keypoints);
    }
    if (pose.keypoints3D != null && params.STATE.modelConfig.render3D) {
      window.keypoints = pose.keypoints3D;
  ...
  drawKeypoint(keypoint) {
    ...
    if (score >= scoreThreshold) {
      ...
      // this.ctx.fillText(`(x: ${Math.round(keypoint.x * 100) / 100}, y: ${Math.round(keypoint.y * 100) / 100})`, keypoint.x, keypoint.y);
    }
  }
